...
def create_container(id_app):
 ...
 \
        ...
        file.write(text)
    docker_command = f'docker build app/docker_task/{id_app}/. -t app-{id_app}'
    run(docker_command.split())
    run(f'docker run --name app-{id_app} -d app-{id_app}'.split())
    app = App.objects.get(pk=id_app)
    status = AppStatus.objects.get(pk=get_status(f'app-{id_app}'))
    app.app_status = status
    app.docker_id = client.containers.get(f'app-{id_app}').short_id
    app.save()

    shutil.rmtree(f'app/docker_task/{id_app}')

    return f'Container app-{id_app} created'


@shared_task
def start_container(id_app):
    container = start_app(f'app-{id_app}')
    return f'Container app-{container.name} started'


@shared_task
def stop_container(id_app):
    container = stop_app(f'app-{id_app}')
    return f'Container app-{container.name} stopped'


@shared_task
def restart_container(id_app):
    container = restart_app(f'app-{id_app}')

    app = App.objects.get(pk=id_app)
    app.app_status = AppStatus.objects.get(pk=2)
    app.save()
    return f'Container app-{container.name} restarted'


@shared_task
def delete_container(id_app):
    delete_app(f'app-{id_app}')
    return f'Container app-{id_app} delited'
